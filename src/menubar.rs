...
pub const TAG_NEXT: isize = 1;
pub const TAG_PREV: isize = 2;
pub const TAG_TOGGLE_FAV: isize = 5;
...
impl MenuBar {
    ...
    pub fn update_state(&mut self, config: &Config, preset_name: &str) {
        self.current_preset_item
            .setTitle(&NSString::from_str(&format!("Preset: {preset_name}")));

        self.toggle_fav_item
            .setState(if config.favorites.contains(preset_name) {
    ...
    pub fn handle_pending_action(&mut self, config: &mut Config) -> i32 {
        ...
        match tag {
            t if t == TAG_TOGGLE_FAV as i32 => {
                // handled in main.rs via gallery
            }
